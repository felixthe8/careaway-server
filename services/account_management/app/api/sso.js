  ...
  try{
    ...
    DB.then(database => {
      ...
      userRepo.FindUser(ssoUser.payload.username).then(function(value){
        var queriedUser = value.User[0].username;
        if(queriedUser === ssoUser.payload.username){
          console.log('User already exist in our system');
          res.status(404);
          res.json({'err' : 'User already exist'});
        } else {
          // saves all salt to an object
          var identifier = new Salt(ssoUser.payload.salt);
          // generates a new user using the received username and password from sso
          var newUser = new User(ssoUser.payload.username,ssoUser.payload.password, {role: 'SSO'}, {} ,identifier);
